...
if( ! class_exists( 'CMB2_Field_Ajax_Search' ) ) {
	...
	class CMB2_Field_Ajax_Search {
		...
		public function __construct() {
			...
			// Ajax endpoints.
			add_action( 'wpfi_ajax_cmb_ajax_search_get_results', array( $this, 'get_results' ) );
		...
		public function get_results() {
			...
			if ( ! wp_verify_nonce( $nonce, 'cmb_ajax_search_get_results' ) ) {
				...
				die( json_encode( array(
                    'error' => __( 'Error : Unauthorized action', 'wp-freeio' )
                ) ) );
                ...
                die( json_encode( array(
                    'error' => __( 'Error : Wrong request parameters', 'wp-freeio' )
                ) ) );
            } else {
		...
		public function object_text( $field_id, $object_id, $object_type ) {
			...
			if( $object_type == 'post' ) {
				...
				if ( WP_Freeio_User::is_employer($object_id) ) {
					$employer_id = WP_Freeio_User::get_employer_by_user_id($object_id);
