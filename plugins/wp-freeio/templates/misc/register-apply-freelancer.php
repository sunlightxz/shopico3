		...
		<?php endif; ?>
  		...
  		<?php
			...
			echo cmb2_get_metabox_form( $metaboxes_form, $post_id, array(
				'form_format' => '<form action="" class="cmb-form" method="post" id="%1$s_apply" enctype="multipart/form-data" encoding="multipart/form-data"><input type="hidden" name="'.$form_obj->get_form_name().'" value="'.$form_obj->get_form_name().'"><input type="hidden" name="job_id" value="'.$job_id.'"><input type="hidden" name="object_id" value="%2$s">%3$s'.$html_output.'<input type="submit" name="submit-cmb-register-apply-freelancer" value="%4$s" class="button-primary"></form>',
