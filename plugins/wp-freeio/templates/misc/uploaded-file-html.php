...
?>
<div class="wp-freeio-uploaded-file">
	<?php
	...
	if ( $value ) {
		if ( is_numeric( $value ) ) {
			...
			$file_url = $value;
			$value = WP_Freeio_Image::get_attachment_id_from_url($value);
	...
	if ( $mime_type ) {
		switch ($mime_type) {
			...
			case 'image/gif':
				?>
				<span class="wp-freeio-uploaded-file-preview"><img src="<?php echo $file_url; ?>" /> <a class="wp-freeio-remove-uploaded-file" href="#">[<?php _e( 'remove', 'wp-freeio' ); ?>]</a></span>
				<?php
				break;
			default:
				?>
				<div class="wp-freeio-uploaded-file-name flex-middle">
					<?php
						$file_info = pathinfo($file_url);
					?>
					<a href="javascript:void(0);" class="freelancer-detail-cv">
					...
					<a class="wp-freeio-remove-uploaded-file" href="#">[<?php _e( 'remove', 'wp-freeio' ); ?>]</a>
				</div>
				<?php
				break;
		}
		...
		<div class="wp-freeio-uploaded-file-name flex-middle">
			<a href="javascript:void(0);" class="freelancer-detail-cv"><?php echo esc_html( basename( $file_url ) ); ?></a>
			<a class="wp-freeio-remove-uploaded-file" href="#">[<?php _e( 'remove', 'wp-freeio' ); ?>]</a>
		</div>
