...
<div class="box-applicants widget">
	<h3 class="widget-title"><?php echo esc_html__('Applicant','wp-freeio') ?></h3>
		...
		<div class="search-applicants-form widget-search">
			<form action="" method="get">
				<div class="input-group">
					...
					<select name="job_id">
						<option value=""><?php esc_html_e('Filter by job', 'wp-freeio'); ?></option>
		...
		foreach ($job_ids as $job_id) {
			...
			$query_vars = array(
				...
				'meta_query'       => array(
					...
					array(
						'key' => WP_FREEIO_APPLICANT_PREFIX.'rejected',
						'value'     => '1',
						'compare'   => '=',
					)
				)
			...
			$query_vars = array(
				...
				'meta_query'       => array(
					...
					array(
						'key' => WP_FREEIO_APPLICANT_PREFIX.'approved',
						'value'     => '1',
						'compare'   => '=',
					)
				)
			...
			<div class="job-applicants">
				<div class="heading">
					...
					<div class="total-applicants show-total-applicants active" data-job_id="<?php echo esc_attr($job_id); ?>" data-nonce="<?php echo esc_attr(wp_create_nonce( 'wp-freeio-show-applicants-nonce' )); ?>">
						<?php echo sprintf(__('Total(s): %s', 'wp-freeio'), '<span class="number">'.$applicants->found_posts.'</span>'); ?>
					</div>
					<div class="approved-applicants show-approved-applicants" data-job_id="<?php echo esc_attr($job_id); ?>" data-nonce="<?php echo esc_attr(wp_create_nonce( 'wp-freeio-show-approved-applicants-nonce' )); ?>">
						<?php echo sprintf(__('Approved: %s', 'wp-freeio'), '<span class="number">'.$approved->found_posts.'</span>'); ?>
					</div>
					<div class="rejected-applicants show-rejected-applicants" data-job_id="<?php echo esc_attr($job_id); ?>" data-nonce="<?php echo esc_attr(wp_create_nonce( 'wp-freeio-show-rejected-applicants-nonce' )); ?>">
						<?php echo sprintf(__('Rejected(s): %s', 'wp-freeio'), '<span class="number">'.$rejected->found_posts.'</span>'); ?>
					</div>
				</div>
