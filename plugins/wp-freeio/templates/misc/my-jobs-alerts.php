...
<div class="box-employer widget">
	<h3 class="widget-title"><?php echo esc_html__('Job Alerts','wp-freeio') ?></h3>
			...
			<table class="job-table">
				...
				<?php foreach ($alerts->posts as $alert_id) {
					...
					<tr <?php post_class('job-alert-wrapper'); ?>>
						...
						<td>
							<div class="job-found">
					            <?php echo sprintf(esc_html__('Jobs found %d', 'wp-freeio'), intval($count_jobs) ); ?>
					        </div>
						</td>
						<td>
							<div class="job-metas">
						...
						<td>
							<a href="javascript:void(0)" class="btn-remove-job-alert btn-action-icon deleted btn-action-sm" data-alert_id="<?php echo esc_attr($alert_id); ?>" data-nonce="<?php echo esc_attr(wp_create_nonce( 'wp-freeio-remove-job-alert-nonce' )); ?>"><i class="flaticon-rubbish-bin"></i></a>
						</td>
					</tr>
	...
	<?php } else { ?>
		<div class="not-found"><?php esc_html_e('No job alert found.', 'wp-freeio'); ?></div>
	<?php } ?>
</div>