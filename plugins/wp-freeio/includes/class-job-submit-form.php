...
class WP_Freeio_Job_Submit_Form extends WP_Freeio_Job_Abstract_Form {
	public $form_name = 'wp_freeio_job_submit_form';
	...
	public function __construct() {
		...
		do_action('wp_freeio_submit_job_construct', $this);
		add_filter( 'cmb2_meta_boxes', array( $this, 'fields_front' ) );
		
	}

	public function get_steps() {
		$this->steps = apply_filters( 'wp_freeio_submit_job_steps', array(
			'submit'  => array(
	...
	public function submit_process() {
		...
		if ( isset( $_POST[ WP_FREEIO_JOB_LISTING_PREFIX . 'title' ] ) ) {
			$post_id = $this->job_id;
			...
			if ( !empty( $post_id ) ) {
				...
				if ( apply_filters('wp-freeio-update-slug-submit-job', true) ) {
					$job_slug = array();

					// Prepend with company name.
					$employer_slug = wp_freeio_get_option('submission_job_slug_employer', 'on');
					if ( $employer_slug == 'on' ) {
						$employer_slug = true;
					} else {
						$employer_slug = false;
					...
					if ( apply_filters( 'wp-freeio-submit-job-form-prefix-post-name-with-company', $employer_slug ) && !empty( $employer_name ) ) {
						$job_slug[] = $employer_name;
					}

					// Prepend location.
					$location_slug = wp_freeio_get_option('submission_job_slug_location', 'on');
					if ( $location_slug == 'on' ) {
						$location_slug = true;
					} else {
						$location_slug = false;
					}
					if ( apply_filters( 'wp-freeio-submit-job-form-prefix-post-name-with-location', $location_slug ) && !empty($_POST[WP_FREEIO_JOB_LISTING_PREFIX.'location']) ) {
						$slugs = $_POST[WP_FREEIO_JOB_LISTING_PREFIX.'location'];
						if ( is_array($slugs) ) {
							foreach ($slugs as $slug) {
								if ( is_numeric($slug) ) {
									$term = get_term($slug, 'location');
									if ( $term && $term->slug ) {
										$job_slug[] = $term->slug;
									}
								} else {
					...
					$type_slug = wp_freeio_get_option('submission_job_slug_type', 'on');
					if ( $type_slug == 'on' ) {
						$type_slug = true;
					} else {
						$type_slug = false;
					}
					if ( apply_filters( 'wp-freeio-submit-job-form-prefix-post-name-with-job-type', $type_slug ) && !empty($_POST[WP_FREEIO_JOB_LISTING_PREFIX.'type']) ) {
						$slugs = $_POST[WP_FREEIO_JOB_LISTING_PREFIX.'type'];
						if ( is_array($slugs) ) {
							foreach ($slugs as $slug) {
								if ( is_numeric($slug) ) {
			...
			if ( ! empty( $post_id ) ) {
				...
				$employer_id = WP_Freeio_User::get_employer_by_user_id($employer_user_id);
				$author_id = update_post_meta($post_id, WP_FREEIO_JOB_LISTING_PREFIX . 'employer_posted_by', $employer_id);
				...
				if( $new_post ) {
					$this->errors[] = __( 'Can not create job', 'wp-freeio' );
				} else {
	...
	public function preview_output() {
		...
		if ( $this->job_id ) {
			...
			do_action('wp-freeio-before-preview-job', $post);

			echo WP_Freeio_Template_Loader::get_template_part( 'submission/job-submit-preview', array(
				'post_id' => $this->job_id,
				'job_id'         => $this->job_id,
				'step'           => $this->get_step(),
				'form_obj'           => $this,
	...
	public function preview_process() {
		...
		if ( !isset( $_POST['security-job-submit-preview'] ) || ! wp_verify_nonce( $_POST['security-job-submit-preview'], 'wp-freeio-job-submit-preview-nonce' )  ) {
		...
		if ( isset( $_POST['continue-edit-job'] ) ) {
			...
			if ( in_array( $job->post_status, array( 'preview', 'expired' ), true ) ) {
				// Reset expiry.
				delete_post_meta( $job->ID, WP_FREEIO_JOB_LISTING_PREFIX.'expiry_date' );
				...
				$update_job['post_status']   = apply_filters( 'wp_freeio_submit_job_post_status', $post_status, $job );
	...
	public function done_output() {
		...
		echo WP_Freeio_Template_Loader::get_template_part( 'submission/job-submit-done', array(
			'post_id' => $this->job_id,
			'job'	  => $job,
		) );
	}
	...
	public function done_handler() {
		do_action( 'wp_freeio_job_submit_done', $this->job_id );
		
		if ( ! empty( $_COOKIE['job_add_new_update'] ) ) {
			$job_add_new_update = $_COOKIE['job_add_new_update'];

			if ( wp_freeio_get_option('admin_notice_add_new_job') ) {
				$job = get_post($this->job_id);
				...
				$subject = WP_Freeio_Email::render_email_vars(array('job' => $job), 'admin_notice_add_new_job', 'subject');
				$content = WP_Freeio_Email::render_email_vars(array('job' => $job), 'admin_notice_add_new_job', 'content');
...
function wp_freeio_job_submit_form() {
	if ( ! empty( $_POST['wp_freeio_job_submit_form'] ) ) {
		WP_Freeio_Job_Submit_Form::get_instance();
	}
}

add_action( 'init', 'wp_freeio_job_submit_form' );