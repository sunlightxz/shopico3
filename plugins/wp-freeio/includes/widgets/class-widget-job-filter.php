...
class WP_Freeio_Widget_Job_Filter extends WP_Widget {
	/**
	 * Initialize widget
	 *
	 * @access public
	...
	function __construct() {
		parent::__construct(
			'job_filter_widget',
			__( 'Job Filter', 'wp-freeio' ),
			array(
				'description' => __( 'Filter for filtering jobs.', 'wp-freeio' ),
			)
		);
	}
	...
	function widget( $args, $instance ) {
		include WP_Freeio_Template_Loader::locate( 'widgets/job-filter' );
	}
	...
	function form( $instance ) {
		...
		<ul class="wp-freeio-filter-fields wp-freeio-filter-job-fields">
			<?php

			$all_fields = $fields = WP_Freeio_Job_Filter::get_fields();
			if ( ! empty( $sort ) ) {
		...
		<script type="text/javascript">
			jQuery(document).ready(function($) {
				$('.widget .wp-freeio-filter-job-fields').each(function() {
					var el = $(this);
...
register_widget('WP_Freeio_Widget_Job_Filter');