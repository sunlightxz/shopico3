...
class WP_Freeio_Project_Edit_Form extends WP_Freeio_Project_Abstract_Form {
	
	public $form_name = 'wp_freeio_project_edit_form';
	...
	public function submit_process() {
		...
		if ( isset( $_POST[ WP_FREEIO_PROJECT_PREFIX . 'title' ] ) && !empty($this->project_id) ) {
			...
			if ( $old_post_status === 'draft' ) {
				...
				$review_before = wp_freeio_get_option( 'user_edit_published_submission_project' );
				$post_status = 'publish';
			...
			do_action( 'wp-freeio-process-edit-project-before-save', $post_id, $this );

			$data = apply_filters('wp-freeio-process-edit-project-data', $data, $post_id);
			...
			if ( ! empty( $post_id ) && ! empty( $_POST['object_id'] ) ) {
				...
				do_action( 'wp-freeio-process-edit-project-after-save', $post_id );
				
				// send email
				if ( wp_freeio_get_option('admin_notice_updated_project') ) {
					...
					$subject = WP_Freeio_Email::render_email_vars(array('project' => $project), 'admin_notice_updated_project', 'subject');
					$content = WP_Freeio_Email::render_email_vars(array('project' => $project), 'admin_notice_updated_project', 'content');
...
function wp_freeio_project_edit_form() {
	if ( ! empty( $_POST['wp_freeio_project_edit_form'] ) ) {
		WP_Freeio_Project_Edit_Form::get_instance();
	}
}

add_action( 'init', 'wp_freeio_project_edit_form' );