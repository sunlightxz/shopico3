...
class WP_Freeio_Custom_Fields_Display {
	...
	public static function hooks() {
        $prefixs = array(WP_FREEIO_JOB_LISTING_PREFIX, WP_FREEIO_EMPLOYER_PREFIX, WP_FREEIO_FREELANCER_PREFIX, WP_FREEIO_SERVICE_PREFIX, WP_FREEIO_PROJECT_PREFIX);
        foreach ($prefixs as $prefix) {
            $hooks = WP_Freeio_Fields_Manager::get_display_hooks($prefix);
	...
	public static function display_hook($post, $current_hook, $prefix) {
		$custom_fields = WP_Freeio_Fields_Manager::get_custom_fields_data($prefix);
	...
	public static function display_field_data($custom_field, $post, $current_hook, $prefix) {
		...
		switch ( $field_type ) {
            ...
            case 'date':
                ...
                break;
            case 'checkbox':
            	$output_value = $value ? esc_html__('Yes', 'wp-freeio') : esc_html__('No', 'wp-freeio');
            	break;
            case 'multiselect':
                if ( is_array($value) ) {
                ...
                if ( is_array($value) ) {
                	foreach ($value as $file) {
                		if ( self::check_image_mime_type($file) ) {
                			...
                			$return .= '<a href="'.esc_url($file).'">'.esc_html__('Download file', 'wp-freeio').'</a>';
                		}
                	}
                } elseif ( !empty($value) ) {
                	if ( self::check_image_mime_type($value) ) {
            			...
            			$return .= '<a href="'.esc_url($value).'">'.esc_html__('Download file', 'wp-freeio').'</a>';
            		}
                }
                $output_value = $return;
            break;
        ...
        <?php
        $html = ob_get_clean();
        return apply_filters( 'wp_freeio_display_field_data', $html, $custom_field, $post, $field_name, $output_value, $current_hook );
	}
...
WP_Freeio_Custom_Fields_Display::init();