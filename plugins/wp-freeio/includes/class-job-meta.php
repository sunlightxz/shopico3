...
class WP_Freeio_Job_Listing_Meta {
	...
	public function get_post_metas() {
		$return = array();
		$fields = WP_Freeio_Custom_Fields::get_custom_fields(array(), 'all', 0, WP_FREEIO_JOB_LISTING_PREFIX);
		if ( !empty($fields) ) {
		...
		return apply_filters('wp-freeio-get-job_listing-post-metas', $return);
	}
	...
	public function check_post_meta_exist($key) {
		if ( isset($this->metas[WP_FREEIO_JOB_LISTING_PREFIX.$key]) ) {
			return true;
		}
		return false;
	}
	...
	public function get_post_meta($key) {
		return get_post_meta($this->post_id, WP_FREEIO_JOB_LISTING_PREFIX.$key, true);
	}
	...
	public function get_post_meta_title($key) {
		if ( !empty($this->metas[WP_FREEIO_JOB_LISTING_PREFIX.$key]) && isset($this->metas[WP_FREEIO_JOB_LISTING_PREFIX.$key]['name'])) {
			return $this->metas[WP_FREEIO_JOB_LISTING_PREFIX.$key]['name'];
		}
		return '';
	}
	...
	public function get_meta_field($key) {
		if ( !empty($this->metas[WP_FREEIO_JOB_LISTING_PREFIX.$key]) ) {
			return $this->metas[WP_FREEIO_JOB_LISTING_PREFIX.$key];
		}
		return '';
	}
