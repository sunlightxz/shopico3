...
class WP_Freeio_Careerbuilder_Jobs_Hooks {

    private static $key = 'wp_freeio_careerbuilder_import';
    ...
    public static function init() {
        ...
        add_action( 'wp_ajax_wp_freeio_ajax_careerbuilder_job_import', array( __CLASS__, 'process_import_jobs' ) );
    ...
    public static function admin_menu() {
        if ( wp_freeio_get_option('careerbuilder_job_import_enable') || (isset($_POST['careerbuilder_job_import_enable']) && $_POST['careerbuilder_job_import_enable'] == 'on') ) {
            add_submenu_page( 'edit.php?post_type=job_listing', esc_html__('Import Careerbuilder Jobs', 'wp-freeio'), esc_html__('Import Careerbuilder Jobs', 'wp-freeio'), 'manage_options', 'import-careerbuilder-jobs', array( __CLASS__, 'jobs_settings' ) );
        }
    }
    ...
    public static function jobs_settings() {
        ...
        <div class="wrap wp_freeio_settings_page cmb2_options_page">
            <h2><?php esc_html_e('Import Careerbuilder Jobs', 'wp-freeio'); ?></h2>
    ...
    public static function import_careerbuilder_fields() {
        $fields = array(
            'id'         => 'options_page',
            'wp_freeio_title' => __( 'Careerbuilder Jobs Import', 'wp-freeio' ),
            'show_on'    => array( 'key' => 'options-page', 'value' => array( self::$key, ), ),
            'fields'     => apply_filters( 'wp_freeio_careerbuilder_job_import_fields', array(
                    ...
                    array(
                        'name'    => __( 'Number of jobs', 'wp-freeio' ),
                        'desc'    => __( 'Enter number of jobs.', 'wp-freeio' ),
            ...
            $metaboxes[ $prefix . 'careerbuilder_job_fields' ] = array(
                ...
                'fields'                    => array(
                    array(
                        ...
                        'id'                => WP_FREEIO_JOB_LISTING_PREFIX . 'careerbuilder_detail_url',
                        'type'              => 'text',
                    ),
                    array(
                        'name'              => __( 'Company Name', 'wp-freeio' ),
                        'id'                => WP_FREEIO_JOB_LISTING_PREFIX . 'careerbuilder_company_name',
                        'type'              => 'text',
                    ),
                ),
            );
    ...
    public static function process_import_jobs() {
        ...
        $careerbuilder_jobs = WP_Freeio_Careerbuilder_API::get_jobs($api_args);
        ...
        if (isset($careerbuilder_jobs['error']) && $careerbuilder_jobs['error'] != '') {
            ...
            foreach ($careerbuilder_jobs as $careerbuilder_job) {
                ...
                $existing_id = WP_Freeio_Mixes::get_post_id_by_meta_value( WP_FREEIO_JOB_LISTING_PREFIX.'careerbuilder_detail_url', $job_url);
                ...
                if ( $post_author ) {
                    ...
                    $user_id = WP_Freeio_User::generate_user_by_post_name($careerbuilder_job['company']);
                    if ( $user_id ) {
                ...
                if (!empty($careerbuilder_job['location'])) {
                    ...
                    add_post_meta($post_id, WP_FREEIO_JOB_LISTING_PREFIX . 'address', $careerbuilder_job['location'], true);
                } else {
                ...
                if (!empty($careerbuilder_job['latitude'])) {
                    add_post_meta($post_id, WP_FREEIO_JOB_LISTING_PREFIX . 'map_location_latitude', esc_attr($careerbuilder_job['latitude']), true);
                    $location_addrs['latitude'] = $careerbuilder_job['latitude'];
                }

                // Insert job longitude meta key
                if (!empty($careerbuilder_job['longitude'])) {
                    add_post_meta($post_id, WP_FREEIO_JOB_LISTING_PREFIX . 'map_location_longitude', esc_attr($careerbuilder_job['longitude']), true);
                ...
                add_post_meta($post_id, WP_FREEIO_JOB_LISTING_PREFIX . 'job_referral', 'careerbuilder', true);

                // Insert job detail url meta key
                if ( $job_url ) {
                    add_post_meta($post_id, WP_FREEIO_JOB_LISTING_PREFIX . 'careerbuilder_detail_url', $job_url, true);
                ...
                if ( !empty($careerbuilder_job['company']) ) {
                    add_post_meta($post_id, WP_FREEIO_JOB_LISTING_PREFIX . 'careerbuilder_company_name', $careerbuilder_job['company'], true);
                }
            ...
            $json = array(
                'status' => false,
                'msg' => sprintf(__('%s careerbuilder jobs are imported successfully.', 'wp-freeio'), count($careerbuilder_jobs))
            );
            echo json_encode($json);
            die();
        }
    ...
    public static function get_company_name($ouput, $post) {
        ...
        $careerbuilder_company_name = get_post_meta($post->ID, WP_FREEIO_JOB_LISTING_PREFIX . 'careerbuilder_company_name', true);
    ...
    public static function get_company_name_html($ouput, $post) {
        ...
        $careerbuilder_company_name = get_post_meta($post->ID, WP_FREEIO_JOB_LISTING_PREFIX . 'careerbuilder_company_name', true);
        if ($job_referral == 'careerbuilder' && $careerbuilder_company_name != '') {
            $ouput = sprintf(wp_kses(__('<span class="employer text-theme">%s</span>', 'wp-freeio'), array( 'span' => array('class' => array()) ) ), $careerbuilder_company_name );
        }
        return $ouput;
    }
    ...
    public static function view_more_btn($post) {
        $ouput = '';
        $job_referral = get_post_meta($post->ID, WP_FREEIO_JOB_LISTING_PREFIX . 'job_referral', true);
        $careerbuilder_detail_url = get_post_meta($post->ID, WP_FREEIO_JOB_LISTING_PREFIX . 'careerbuilder_detail_url', true);
        if ($job_referral == 'careerbuilder' && $careerbuilder_detail_url != '') {
            $ouput = '<div class="view-more-link"><a class="btn btn-theme" href="' . $careerbuilder_detail_url . '">' . esc_html__('view more', 'wp-freeio') . '</a></div>';
        }
        return $ouput;
    }
}

add_filter( 'cmb2_get_metabox_form_format', 'wp_freeio_careerbuilder_modify_cmb2_form_output', 10, 3 );

function wp_freeio_careerbuilder_modify_cmb2_form_output( $form_format, $object_id, $cmb ) {
    if ( 'wp_freeio_careerbuilder_import' == $object_id && 'options_page' == $cmb->cmb_id ) {

        return '<form class="cmb-form" method="post" id="%1$s" enctype="multipart/form-data" encoding="multipart/form-data"><input type="hidden" name="object_id" value="%2$s">%3$s<div class="wp_freeio-submit-wrap"><input type="button" name="submit-cmb-careerbuilder-job-import" value="' . __( 'Import Careerbuilder Jobs', 'wp-freeio' ) . '" class="button-primary"></div></form>';
    }

    return $form_format;
}

WP_Freeio_Careerbuilder_Jobs_Hooks::init();