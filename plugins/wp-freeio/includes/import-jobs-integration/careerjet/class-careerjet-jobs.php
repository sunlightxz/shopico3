...
class WP_Freeio_Careerjet_Jobs_Hooks {

    private static $key = 'wp_freeio_careerjet_import';
    ...
    public static function init() {
        ...
        add_action( 'wp_ajax_wp_freeio_ajax_careerjet_job_import', array( __CLASS__, 'process_import_careerjet_jobs' ) );
    ...
    public static function careerjet_jobs_admin_menu() {
        if ( wp_freeio_get_option('careerjet_job_import_enable') || (isset($_POST['careerjet_job_import_enable']) && $_POST['careerjet_job_import_enable'] == 'on') ) {
            add_submenu_page( 'edit.php?post_type=job_listing', esc_html__('Import Careerjet Jobs', 'wp-freeio'), esc_html__('Import Careerjet Jobs', 'wp-freeio'), 'manage_options', 'import-careerjet-jobs', array( __CLASS__, 'import_careerjet_jobs_settings' ) );
        }
    }
    ...
    public static function import_careerjet_jobs_settings() {
        ...
        <div class="wrap wp_freeio_settings_page cmb2_options_page">
            <h2><?php esc_html_e('Import Careerjet Jobs', 'wp-freeio'); ?></h2>
    ...
    public static function import_careerjet_fields() {
        $fields = array(
            'id'         => 'options_page',
            'wp_freeio_title' => __( 'Careerjet Jobs Import', 'wp-freeio' ),
            'show_on'    => array( 'key' => 'options-page', 'value' => array( self::$key, ), ),
            'fields'     => apply_filters( 'wp_freeio_careerjet_job_import_fields', array(
                    ...
                    array(
                        'name'    => __( 'Page', 'wp-freeio' ),
                        'desc'    => __( 'Enter page number to import jobs. Default page number is 1.', 'wp-freeio' ),
            ...
            $metaboxes[ $prefix . 'careerjet_job_fields' ] = array(
                ...
                'fields'                    => array(
                    array(
                        ...
                        'id'                => WP_FREEIO_JOB_LISTING_PREFIX . 'careerjet_detail_url',
                        'type'              => 'text',
                    ),
                    array(
                        'name'              => __( 'Company Name', 'wp-freeio' ),
                        'id'                => WP_FREEIO_JOB_LISTING_PREFIX . 'careerjet_company_name',
                        'type'              => 'text',
                    ),
                ),
            );
    ...
    public static function process_import_careerjet_jobs() {
        ...
        $careerjet_jobs = WP_Freeio_Careerjet_API::get_jobs($api_args);
        ...
        if (isset($careerjet_jobs['error']) && $careerjet_jobs['error'] != '') {
            ...
            foreach ($careerjet_jobs as $careerjet_job) {
                ...
                $existing_id = WP_Freeio_Mixes::get_post_id_by_meta_value( WP_FREEIO_JOB_LISTING_PREFIX.'careerjet_detail_url', $job_url);
                ...
                if ( $post_author ) {
                    ...
                    $user_id = WP_Freeio_User::generate_user_by_post_name($careerjet_job->company);
                    if ( $user_id ) {
                ...
                if (!empty($careerjet_job->locations)) {
                    ...
                    update_post_meta($post_id, WP_FREEIO_JOB_LISTING_PREFIX . 'address', $careerjet_job->locations, true);
                } else {
                    $location_addrs['address'] = '';
                }

                update_post_meta($post_id, WP_FREEIO_JOB_LISTING_PREFIX . 'map_location', $location_addrs, true);

                // Insert job referral meta key
                update_post_meta($post_id, WP_FREEIO_JOB_LISTING_PREFIX . 'job_referral', 'careerjet', true);

                // Insert job detail url meta key
                update_post_meta($post_id, WP_FREEIO_JOB_LISTING_PREFIX . 'careerjet_detail_url', esc_url($job_url), true);
                ...
                update_post_meta($post_id, WP_FREEIO_JOB_LISTING_PREFIX . 'careerjet_company_name', $job_company, true);

                // salary
                if ( !empty($careerjet_job->salary_min) ) {
                    update_post_meta($post_id, WP_FREEIO_JOB_LISTING_PREFIX . 'salary', $careerjet_job->salary_min, true);
                }

                if ( !empty($careerjet_job->salary_max) ) {
                    update_post_meta($post_id, WP_FREEIO_JOB_LISTING_PREFIX . 'max_salary', $careerjet_job->salary_max, true);
                }
                
            }
            $json = array(
                'status' => true,
                'msg' => sprintf(__('%s careerjet jobs are imported successfully.', 'wp-freeio'), count($careerjet_jobs))
            );
            echo json_encode($json);
            die();
        }
    ...
    public static function get_company_name($ouput, $post) {
        ...
        $careerjet_company_name = get_post_meta($post->ID, WP_FREEIO_JOB_LISTING_PREFIX . 'careerjet_company_name', true);
    ...
    public static function get_company_name_html($ouput, $post) {
        ...
        $careerjet_company_name = get_post_meta($post->ID, WP_FREEIO_JOB_LISTING_PREFIX . 'careerjet_company_name', true);
        if ($job_referral == 'careerjet' && $careerjet_company_name != '') {
            $ouput = sprintf(wp_kses(__('<span class="employer text-theme">%s</span>', 'wp-freeio'), array( 'span' => array('class' => array()) ) ), $careerjet_company_name );
        }
        return $ouput;
    }
    ...
    public static function view_more_btn($post) {
        $ouput = '';
        $job_referral = get_post_meta($post->ID, WP_FREEIO_JOB_LISTING_PREFIX . 'job_referral', true);
        $careerjet_detail_url = get_post_meta($post->ID, WP_FREEIO_JOB_LISTING_PREFIX . 'careerjet_detail_url', true);
        if ($job_referral == 'careerjet' && $careerjet_detail_url != '') {
            $ouput = '<div class="view-more-link"><a class="btn btn-theme" href="' . $careerjet_detail_url . '">' . esc_html__('view more', 'wp-freeio') . '</a></div>';
        }
        return $ouput;
    }
}

add_filter( 'cmb2_get_metabox_form_format', 'wp_freeio_careerjet_modify_cmb2_form_output', 10, 3 );

function wp_freeio_careerjet_modify_cmb2_form_output( $form_format, $object_id, $cmb ) {
    if ( 'wp_freeio_careerjet_import' == $object_id && 'options_page' == $cmb->cmb_id ) {

        return '<form class="cmb-form" method="post" id="%1$s" enctype="multipart/form-data" encoding="multipart/form-data"><input type="hidden" name="object_id" value="%2$s">%3$s<div class="wp_freeio-submit-wrap"><input type="button" name="submit-cmb-careerjet-job-import" value="' . __( 'Import Careerjet Jobs', 'wp-freeio' ) . '" class="button-primary"></div></form>';
    }

    return $form_format;
}

WP_Freeio_Careerjet_Jobs_Hooks::init();