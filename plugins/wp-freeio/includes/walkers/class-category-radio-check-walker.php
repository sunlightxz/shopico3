	...
	public function start_el( &$output, $object, $depth = 0, $args = [], $current_object_id = 0 ) {
		...
		$output .= '<li class="list-item level-' . intval( $depth ) . '">';
        	...
        	if ( $args['input_type'] == 'checkbox' ) {
	        	$output .= '<input id="'.esc_attr($args['name'].'-'.$object->slug.'-'.$rand_id).'" type="checkbox" name="'.esc_attr($args['name']).'[]" value="'.esc_attr($value).'" '.$checked.'>';
	        } else {
	        	$output .= '<input id="'.esc_attr($args['name'].'-'.$object->slug.'-'.$rand_id).'" type="radio" name="'.esc_attr($args['name']).'" value="'.esc_attr($value).'" '.$checked.'>';
	        }
        	$output .= '<label for="'.esc_attr($args['name'].'-'.$object->slug.'-'.$rand_id).'">'. esc_html( $cat_name.$count ).'</label>';
