...
class WP_Freeio_Post_Type_Job_Alert {
	...
	public static function register_post_type() {
		$singular = __( 'Job Alert', 'wp-freeio' );
		$plural   = __( 'Job Alerts', 'wp-freeio' );
		...
		if ( isset($_GET['post']) && $_GET['post'] && is_admin() ) {
			...
			if ( $post && $post->post_type == 'job_alert' ) {
				...
				$fields[] = array(
					...
					'type' => 'title',
					'id'   => WP_FREEIO_JOB_ALERT_PREFIX . 'author'
				);
			}
		}
		$fields[] = array(
			'name'              => __( 'Alert Query', 'wp-freeio' ),
			'id'                => WP_FREEIO_JOB_ALERT_PREFIX . 'alert_query',
			'type'              => 'textarea',
		);
		$fields[] = array(
			'name'              => __( 'Email Frequency', 'wp-freeio' ),
			'id'                => WP_FREEIO_JOB_ALERT_PREFIX . 'email_frequency',
			'type'              => 'select',
			'options'			=> $email_frequency
		);
		$metaboxes[ WP_FREEIO_JOB_ALERT_PREFIX . 'general' ] = array(
			'id'                        => WP_FREEIO_JOB_ALERT_PREFIX . 'general',
	...
	public static function custom_columns_manage( $column ) {
		switch ( $column ) {
			case 'email_frequency':
					$email_frequency = get_post_meta( get_the_ID(), WP_FREEIO_JOB_ALERT_PREFIX . 'email_frequency', true );
...
WP_Freeio_Post_Type_Job_Alert::init();