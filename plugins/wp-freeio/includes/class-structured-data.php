...
class WP_Freeio_Structured_Data {
	...
	public static function output_structured_data() {
		...
		if ( ! empty( $structured_data ) ) {
			echo '<!-- WP Freeio Structured Data -->' . "\r\n";
	...
	public static function output_job_listing_structured_data( $post = null ) {
		...
		return apply_filters( 'wp_freeio_output_job_listing_structured_data', $output_structured_data, $post );
	}
	...
	public static function get_job_listing_structured_data( $post = null ) {
		...
		if ( ! empty( $location ) ) {
			$data['jobLocation']            = array();
			$data['jobLocation']['@type']   = 'Place';
			$data['jobLocation']['address'] = self::get_location_structured_data( $post );
			if ( empty($data['jobLocation']['address']) ) {
				$data['jobLocation']['address'] = $location;
			}
		}

		$min_salary = WP_Freeio_Job_Listing::get_post_meta( $post->ID, 'salary', true );
		$max_salary = WP_Freeio_Job_Listing::get_post_meta( $post->ID, 'max_salary', true );

		$symbol = wp_freeio_get_option('custom_symbol');
		if ( empty($symbol) ) {
			$currency = wp_freeio_get_option('currency', 'USD');
			$symbol = WP_Freeio_Price::currency_symbol($currency);
		}
		
		$salary_type = WP_Freeio_Job_Listing::get_post_meta( $post->ID, 'salary_type', true );
		$unitText = '';
		switch ($salary_type) {
			case 'yearly':
				$unitText = 'YEAR';
		...
		// expired
		$expiry_date = WP_Freeio_Job_Listing::get_post_meta( $post->ID, 'expiry_date', true );
		if ( $expiry_date ) {
			$data['validThrough'] = $expiry_date;
		}
		
		return apply_filters( 'wp_freeio_get_job_listing_structured_data', $data, $post );
	}

	public static function get_location_structured_data( $post ) {
		...
		$properties = WP_Freeio_Job_Listing::get_post_meta( $post->ID, 'map_location_properties', true );
		...
		return apply_filters( 'wp_freeio_get_location_structured_data', $address, $post );
	}
	...
	public static function get_company_website( $post = null ) {
		...
		if ( $employer_id ) {
			...
			$website = WP_Freeio_Employer::get_post_meta($employer_id, 'website', true);

			if ( $website && ! strstr( $website, 'http:' ) && ! strstr( $website, 'https:' ) ) {
				$website = 'http://' . $website;
			}

			return apply_filters( 'wp_freeio_the_company_website', $website, $post );
		}
		return '';
	}
	...
	public static function get_job_location( $post ) {
		$location = WP_Freeio_Job_Listing::get_post_meta($post->ID, 'address', true);
		if ( empty($location) ) {
			$location = WP_Freeio_Job_Listing::get_post_meta( $post->ID, 'map_location_address', true );
		}
	...
	public static function get_job_employment_types( $post ) {
		...
		return apply_filters( 'wp_freeio_get_job_employment_types', array_unique( $employment_types ), $post );
	}
	...
	public static function get_the_job_types( $post ) {
		...
		return apply_filters( 'wp_freeio_the_job_types', $types, $post );
	}
}

WP_Freeio_Structured_Data::init();