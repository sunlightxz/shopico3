...
class WP_Job_Board_Geocode {
	...
	public static function init() {
		add_filter( 'wp-freeio-geolocation-endpoint', [ __CLASS__, 'add_geolocation_endpoint_query_args' ], 0, 2 );
		add_filter( 'wp-freeio-geolocation-api-key', [ __CLASS__, 'get_google_maps_api_key' ], 0 );
	}
	...
	public static function save_location_data( $job_id, $address_data ) {
		if ( ! is_wp_error( $address_data ) && $address_data ) {
			...
			WP_Freeio_Job_Listing::update_post_meta( $post->ID, 'map_location_latitude', $maps_address['latitude'] );
			WP_Freeio_Job_Listing::update_post_meta( $post->ID, 'map_location_longitude', $maps_address['longitude'] );
			WP_Freeio_Job_Listing::update_post_meta( $post->ID, 'map_location_address', $maps_address['address'] );
			WP_Freeio_Job_Listing::update_post_meta( $post->ID, 'map_location', $maps_address );

			WP_Freeio_Job_Listing::update_post_meta( $post->ID, 'map_location_properties', $address_data );
		}
	}

	public static function get_google_maps_api_key( $key ) {
		if ( wp_freeio_get_option('map_service') == 'google-map' ) {
			return wp_freeio_get_option('google_map_api_keys');
		}
		return '';
	}
	...
	public static function add_geolocation_endpoint_query_args( $geocode_endpoint_url, $raw_address ) {
		...
		$api_key = apply_filters( 'wp-freeio-geolocation-api-key', '', $raw_address );
		...
		$region = apply_filters( 'wp-freeio-geolocation-region-cctld', '', $raw_address );
		if ( '' !== $region ) {
	...
	public static function get_location_data( $raw_address ) {
		...
		$transient_name              = 'wpfi_geocode_' . md5( $raw_address );
		$geocoded_address            = get_transient( $transient_name );
		$wpfi_geocode_over_query_limit = get_transient( 'wpfi_geocode_over_query_limit' );

		// Query limit reached - don't geocode for a while.
		if ( $wpfi_geocode_over_query_limit && false === $geocoded_address ) {
			return false;
		}

		$geocode_api_url = apply_filters( 'wp-freeio-geolocation-endpoint', self::GOOGLE_MAPS_GEOCODE_API_URL, $raw_address );
			...
			if ( false === $geocoded_address || empty( $geocoded_address->results[0] ) ) {
				$result           = wp_remote_get(
					...
					[
						...
						'redirection' => 1,
						'httpversion' => '1.1',
						'user-agent'  => 'WordPress/WP-Job-Board-Pro-' . WP_FREEIO_PLUGIN_VERSION . '; ' . get_bloginfo( 'url' ),
						'sslverify'   => false,
					]
				);
				...
				if ( isset( $geocoded_address->status ) ) {
					if ( 'ZERO_RESULTS' === $geocoded_address->status ) {
						throw new Exception( __( 'No results found', 'wp-freeio' ) );
					} elseif ( 'OVER_QUERY_LIMIT' === $geocoded_address->status ) {
						set_transient( 'wpfi_geocode_over_query_limit', 1, HOUR_IN_SECONDS );
						throw new Exception( __( 'Query limit reached', 'wp-freeio' ) );
						...
						throw new Exception( __( 'Geocoding error', 'wp-freeio' ) );
					}
				} else {
					throw new Exception( __( 'Geocoding error', 'wp-freeio' ) );
				}
		...
		return apply_filters( 'wp-freeio-geolocation-get-location-data', $address, $geocoded_address );
	}
}
