...
class WP_Freeio_Job_Abstract_Form {
	...
	public function fields_front($metaboxes) {
		...
		$init_fields = apply_filters( 'wp-freeio-job_listing-fields-front', array(), $post_id);
		...
		foreach ($init_fields as $field) {
			...
			if ( $i == 1 ) {
				if ( $field['type'] !== 'title' ) {
					$fields[] = array(
						...
						'type' => 'title',
						'id'   => WP_FREEIO_JOB_LISTING_PREFIX.'heading_general_title',
						'priority' => 0,
						'before_row' => '<div id="heading-'.WP_FREEIO_JOB_LISTING_PREFIX.'heading_general_title" class="before-group-row before-group-row-'.$heading_count.' active"><div class="before-group-row-inner">',
					);
					$heading_count = 1;
					$index = 0;
				}
			...
			if ( $rfield['id'] == WP_FREEIO_JOB_LISTING_PREFIX . 'title' ) {
				$rfield['default'] = !empty( $post ) ? $post->post_title : '';
			} elseif ( $rfield['id'] == WP_FREEIO_JOB_LISTING_PREFIX . 'description' ) {
				$rfield['default'] = !empty( $post ) ? $post->post_content : '';
			} elseif ( $rfield['id'] == WP_FREEIO_JOB_LISTING_PREFIX . 'featured_image' ) {
				$rfield['default'] = !empty( $featured_image ) ? $featured_image : '';
			} elseif ( $rfield['id'] == WP_FREEIO_JOB_LISTING_PREFIX . 'tag' ) {
		...
		$fields[] = array(
			'id'                => WP_FREEIO_JOB_LISTING_PREFIX . 'post_type',
			'type'              => 'hidden',
			'default'           => 'job_listing',
			'priority'          => 100,
		);

		$fields = apply_filters( 'wp-freeio-job_listing-fields', $fields, $post_id );

		$metaboxes[ WP_FREEIO_JOB_LISTING_PREFIX . 'front' ] = array(
			'id'                        => WP_FREEIO_JOB_LISTING_PREFIX . 'front',
	...
	public function form_output() {
		...
		if ( ! isset( $metaboxes[ WP_FREEIO_JOB_LISTING_PREFIX . 'front' ] ) ) {
			return __( 'A metabox with the specified \'metabox_id\' doesn\'t exist.', 'wp-freeio' );
		}
		$metaboxes_form = $metaboxes[ WP_FREEIO_JOB_LISTING_PREFIX . 'front' ];
		...
		echo WP_Freeio_Template_Loader::get_template_part( 'submission/job-submit-form', array(
			'post_id' => $this->job_id,
			'metaboxes_form' => $metaboxes_form,
			'job_id'         => $this->job_id,
			'step'           => $this->get_step(),
			'form_obj'       => $this,
			'submit_button_text' => apply_filters( 'wp_freeio_submit_job_form_submit_button_text', $submit_button_text ),
		) );
	}
}
