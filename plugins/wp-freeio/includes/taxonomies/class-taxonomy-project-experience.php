...
class WP_Freeio_Taxonomy_Project_Experience{
	...
	public static function definition($args) {
		$singular = __( 'Experience', 'wp-freeio' );
		$plural   = __( 'Experiences', 'wp-freeio' );
		...
		$rewrite_slug = get_option('wp_freeio_project_experience_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'project-experience', 'Project experience slug - resave permalinks after changing this', 'wp-freeio' );
		}
		$rewrite = array(
			'slug'         => $rewrite_slug,
			'with_front'   => false,
		...
		register_taxonomy( 'project_experience', 'project', array(
			'labels'            => apply_filters( 'wp_freeio_taxomony_project_experience_labels', $labels ),
			'hierarchical'      => true,
			'rewrite'           => $rewrite,
			'public'            => true,
			'show_ui'           => true,
...
WP_Freeio_Taxonomy_Project_Experience::init();