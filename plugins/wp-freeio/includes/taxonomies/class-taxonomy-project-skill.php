...
class WP_Freeio_Taxonomy_Project_Skill{
	...
	public static function definition() {
		$singular = __( 'Skill', 'wp-freeio' );
		$plural   = __( 'Skills', 'wp-freeio' );
		...
		$rewrite_slug = get_option('wp_freeio_project_skill_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'project-skill', 'Project skill slug - resave permalinks after changing this', 'wp-freeio' );
		}
		$rewrite = array(
			'slug'         => $rewrite_slug,
			'with_front'   => false,
		...
		register_taxonomy( 'project_skill', 'project', array(
			'labels'            => apply_filters( 'wp_freeio_taxomony_project_skill_labels', $labels ),
			'hierarchical'      => false,
			'rewrite'           => $rewrite,
			'public'            => true,
			'show_ui'           => true,
...
WP_Freeio_Taxonomy_Project_Skill::init();