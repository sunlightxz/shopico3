...
class WP_Freeio_Taxonomy_Service_Tag{
	...
	public static function definition() {
		...
		$rewrite_slug = get_option('wp_freeio_service_tag_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'service-tag', 'Service tag slug - resave permalinks after changing this', 'wp-freeio' );
		}
		$rewrite = array(
			'slug'         => $rewrite_slug,
			'with_front'   => false,
		...
		register_taxonomy( 'service_tag', 'service', array(
			'labels'            => apply_filters( 'wp_freeio_taxomony_service_tag_labels', $labels ),
			'hierarchical'      => false,
			'rewrite'           => $rewrite,
			'public'            => true,
			'show_ui'           => true,
...
WP_Freeio_Taxonomy_Service_Tag::init();