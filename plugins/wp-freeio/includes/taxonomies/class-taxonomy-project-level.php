...
class WP_Freeio_Taxonomy_Project_Level{
	...
	public static function definition($args) {
		$singular = __( 'Level', 'wp-freeio' );
		$plural   = __( 'Levels', 'wp-freeio' );
		...
		$rewrite_slug = get_option('wp_freeio_project_level_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'project-level', 'Project level slug - resave permalinks after changing this', 'wp-freeio' );
		}
		$rewrite = array(
			'slug'         => $rewrite_slug,
			'with_front'   => false,
		...
		register_taxonomy( 'project_level', 'project', array(
			'labels'            => apply_filters( 'wp_freeio_taxomony_project_level_labels', $labels ),
			'hierarchical'      => true,
			'rewrite'           => $rewrite,
			'public'            => true,
			'show_ui'           => true,
...
WP_Freeio_Taxonomy_Project_Level::init();