...
class WP_Freeio_Taxonomy_Service_Category{
	...
	public static function definition($args) {
		...
		$labels = array(
			'name'              => sprintf(__( 'Service %s', 'wp-freeio' ), $plural),
		...
		$rewrite_slug = get_option('wp_freeio_service_category_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'service-category', 'Service category slug - resave permalinks after changing this', 'wp-freeio' );
		}
		$rewrite = array(
			'slug'         => $rewrite_slug,
			'with_front'   => false,
		...
		register_taxonomy( 'service_category', 'service', array(
			'labels'            => apply_filters( 'wp_freeio_taxomony_service_category_labels', $labels ),
			'hierarchical'      => true,
			'rewrite'           => $rewrite,
			'public'            => true,
			'show_ui'           => true,
...
WP_Freeio_Taxonomy_Service_Category::init();