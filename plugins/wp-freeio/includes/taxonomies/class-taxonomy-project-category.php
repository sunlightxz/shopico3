...
class WP_Freeio_Taxonomy_Project_Category{
	...
	public static function definition($args) {
		...
		$rewrite_slug = get_option('wp_freeio_project_category_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'project-category', 'Project category slug - resave permalinks after changing this', 'wp-freeio' );
		}
		$rewrite = array(
			'slug'         => $rewrite_slug,
			'with_front'   => false,
		...
		register_taxonomy( 'project_category', 'project', array(
			'labels'            => apply_filters( 'wp_freeio_taxomony_project_category_labels', $labels ),
			'hierarchical'      => true,
			'rewrite'           => $rewrite,
			'public'            => true,
			'show_ui'           => true,
...
WP_Freeio_Taxonomy_Project_Category::init();